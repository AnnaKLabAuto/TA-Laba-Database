...
sql.get_all_clients = SELECT id, first_name, last_name, email, phone, company, address FROM clients;
...
sql.get_all_projects = SELECT id_project, project_name, project_description, start_date, due_date, priority, status, client_id, department_id FROM projects;
...
sql.get_all_tasks = SELECT id_task, task_name, task_description, priority, status, project_id FROM tasks;
