sql.create_employee = INSERT INTO employees (first_name, last_name, email, phone, job_title, salary, is_project_manager,employment_statuses_id, leave_types_id, departments_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
sql.update_by_id_employee = UPDATE employees SET first_name=?, last_name=?, email=?, phone=?, job_title=?, salary=?, is_project_manager=?, employment_statuses_id=?, leave_types_id=?, departments_id=? WHERE id_employee=?
...
sql.create_project = INSERT INTO projects (project_name, project_description, start_date, due_date, priority, status, clients_id, departments_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?);
sql.update_by_id_project = UPDATE projects SET project_name=?, project_description=?, start_date=?, due_date=?, priority=?, status=?, clients_id=?, departments_id=? WHERE id_project=?;
...
sql.create_task = INSERT INTO tasks (task_name, task_description, priority, status, project_ids) VALUES (?, ?, ?, ?, ?);
sql.update_by_id_task = UPDATE tasks SET task_name=?, task_description=?, priority=?, status=?, project_ids=? WHERE id_task=?;
